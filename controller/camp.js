...
try{
   const {cadet_regimentNo,campImages}=req.body
   ...
   if(error) return res.send(error.details[0].message);
        ...
        let addData=await Camp.create(payload)
        ...
        if(addData){
          let findStudent=await Student.findOne({regimentNo:cadet_regimentNo})
            console.log("camp len",findStudent.camp);

            tempArr=findStudent.camp
          // if(findStudent.camp){
             tempArr.push(addData._id)
          // }
          console.log("tempArr",tempArr.length);
          const updateStudent=await Student.findOneAndUpdate({regimentNo:cadet_regimentNo},{
            $set:{
              camp:tempArr
            }
          })
...
const getAll=async(req,res)=>{
  ...
  let parseMatch=JSON.parse(match)

  // console.log("parse",parseMatch);
  
  const viewcamp=await Camp.find(parseMatch).sort({start_date:-1})
  res.send(viewcamp)
}
...
const deleteCamp=async(req,res)=>{
  let {match='{}'}=req.query
     ...
     const updateCamp=await Camp.findByIdAndUpdate({_id:parseMatch._id},{new:true})
     console.log("upadate",updateCamp.status
     );
      updateCamp.status=parseMatch.status
      const result=await  updateCamp.save()


// console.log("view",viewcamp);
    return res.json({
      data:updateCamp,
...
Fromdate,Enddate,Camptype,Campplace,Campname,deleteCamp}
